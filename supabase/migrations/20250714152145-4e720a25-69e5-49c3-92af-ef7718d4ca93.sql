...
-- Revert RLS policies to use legacy role column instead of enhanced_role

-- Revert organization_users policies
...
-- Revert question_options policies
...
-- Revert question_scale_config policies
...
-- Revert user_invitations policy
...
-- Revert invitation_events policy
...
-- Revert database functions to use legacy role
...
-- Revert invite_user_to_organization function
CREATE OR REPLACE FUNCTION public.invite_user_to_organization(p_email text, p_organization_id uuid, p_role text DEFAULT 'member'::text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- Revert remove_user_from_organization function
...
-- Revert cancel_invitation function
